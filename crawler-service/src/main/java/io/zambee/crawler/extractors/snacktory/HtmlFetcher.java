...
public class HtmlFetcher {
    ...
    static {
        StringOperations.enableCookieMgmt();
        StringOperations.enableUserAgentOverwrite();
        StringOperations.enableAnySSL();
    }
    ...
    public JResult fetchAndExtract(String url, int timeout, boolean resolve) throws Exception {
        ...
        url = StringOperations.removeHashbang(url);
        String gUrl = StringOperations.getUrlFromUglyGoogleRedirect(url);
        if (gUrl != null)
            url = gUrl;
        else {
            gUrl = StringOperations.getUrlFromUglyFacebookRedirect(url);
            if (gUrl != null)
                url = gUrl;
        }
        ...
        if (resolve) {
            ...
            if (resUrl != null && resUrl.trim().length() > url.length()) {
                ...
                url = StringOperations.useDomainOfFirstArg4Second(url, resUrl);
            }
        }
        ...
        result.setDate(StringOperations.estimateDate(url));

        String lowerUrl = url.toLowerCase();
        if (StringOperations.isDoc(lowerUrl) || StringOperations.isApp(lowerUrl) || StringOperations.isPackage(lowerUrl)) {
            // skip
        } else if (StringOperations.isVideo(lowerUrl) || StringOperations.isAudio(lowerUrl)) {
            result.setVideoUrl(url);
        } else if (StringOperations.isImage(lowerUrl)) {
            result.setImageUrl(url);
        } else {
            extractor.extractContent(result, fetchAsString(url, timeout));
            if (result.getFaviconUrl().isEmpty())
                result.setFaviconUrl(StringOperations.getDefaultFavicon(url));
    ...
    private static String fixUrl(String url, String urlOrPath) {
        return StringOperations.useDomainOfFirstArg4Second(url, urlOrPath);
    }
    ...
    public String getResolvedUrl(String urlAsString, int timeout) {
        ...
        try {
            ...
            if (responseCode / 100 == 3 && newUrl != null) {
                ...
                if (furtherResolveNecessary.contains(StringOperations.extractDomain(newUrl, true)))
