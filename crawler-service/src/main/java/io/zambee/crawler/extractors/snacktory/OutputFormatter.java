...
public class OutputFormatter {
    ...
    public String getFormattedText(Element topNode) {
        ...
        String str = StringOperations.innerTrim(sb.toString());
        if (str.length() > 100)
            return str;
    ...
    public List<String> getTextList(Element topNode) {
        return topNode.select(this.nodesToKeepCssSelector).stream()
                      .filter(Element::hasText).map(Element::text).collect(Collectors.toList());
    }
    ...
    protected void append(Element node, StringBuilder sb, String tagName) {
        ...
        for (Element e : node.select(tagName)) {
            ...
            if (text.isEmpty() || text.length() < minParagraphText || text.length() > StringOperations.countLetters(text) * 2)
                continue;
