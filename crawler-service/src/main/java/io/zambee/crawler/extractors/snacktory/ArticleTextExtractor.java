...
import io.zambee.crawler.comparator.ImageComparator;
...
public class ArticleTextExtractor {
    ...
    public JResult extractContent(JResult res, Document doc, OutputFormatter formatter) throws Exception {
        ...
        if (bestMatchElement != null) {
            ...
            if (imgEl != null) {
                res.setImageUrl(StringOperations.replaceSpaces(imgEl.attr("src")));
...
//                print("best element:", bestMatchElement);
    ...
    protected String extractTitle(Document doc) {
        ...
        if (title.isEmpty()) {
            title = StringOperations.innerTrim(doc.select("head title").text());
            if (title.isEmpty()) {
                title = StringOperations.innerTrim(doc.select("head meta[name=title]").attr("content"));
                if (title.isEmpty()) {
                    title = StringOperations.innerTrim(doc.select("head meta[property=og:title]").attr("content"));
                    if (title.isEmpty()) {
                        title = StringOperations.innerTrim(doc.select("head meta[name=twitter:title]").attr("content"));
                    }
                }
            }
        }
    ...
    protected String extractCanonicalUrl(Document doc) {
        String url = StringOperations.replaceSpaces(doc.select("head link[rel=canonical]").attr("href"));
        if (url.isEmpty()) {
            url = StringOperations.replaceSpaces(doc.select("head meta[property=og:url]").attr("content"));
            if (url.isEmpty()) {
                url = StringOperations.replaceSpaces(doc.select("head meta[name=twitter:url]").attr("content"));
            }
        }
        return url;
    }
    ...
    protected String extractDescription(Document doc) {
        String description = StringOperations.innerTrim(doc.select("head meta[name=description]").attr("content"));
        if (description.isEmpty()) {
            description = StringOperations.innerTrim(doc.select("head meta[property=og:description]").attr("content"));
            if (description.isEmpty()) {
                description = StringOperations.innerTrim(doc.select("head meta[name=twitter:description]").attr("content"));
            }
        }
        return description;
    }
    ...
    protected Collection<String> extractKeywords(Document doc) {
        String content = StringOperations.innerTrim(doc.select("head meta[name=keywords]").attr("content"));
    ...
    protected String extractImageUrl(Document doc) {
        ...
        String imageUrl = StringOperations.replaceSpaces(doc.select("head meta[property=og:image]").attr("content"));
        if (imageUrl.isEmpty()) {
            imageUrl = StringOperations.replaceSpaces(doc.select("head meta[name=twitter:image]").attr("content"));
            if (imageUrl.isEmpty()) {
                // prefer link over thumbnail-meta if empty
                imageUrl = StringOperations.replaceSpaces(doc.select("link[rel=image_src]").attr("href"));
                if (imageUrl.isEmpty()) {
                    imageUrl = StringOperations.replaceSpaces(doc.select("head meta[name=thumbnail]").attr("content"));
                }
            }
        }
        return imageUrl;
    ...
    protected String extractRssUrl(Document doc) {
        return StringOperations.replaceSpaces(doc.select("link[rel=alternate]").select("link[type=application/rss+xml]").attr("href"));
    }

    protected String extractVideoUrl(Document doc) {
        return StringOperations.replaceSpaces(doc.select("head meta[property=og:video]").attr("content"));
    }

    protected String extractFaviconUrl(Document doc) {
        String faviconUrl = StringOperations.replaceSpaces(doc.select("head link[rel=icon]").attr("href"));
        if (faviconUrl.isEmpty()) {
            faviconUrl = StringOperations.replaceSpaces(doc.select("head link[rel^=shortcut],link[rel$=icon]").attr("href"));
        }
        return faviconUrl;
    }
    ...
    private int calcWeightForChild(Element child, String ownText) {
        int c = StringOperations.count(ownText, "&quot;");
        c += StringOperations.count(ownText, "&lt;");
        c += StringOperations.count(ownText, "&gt;");
        c += StringOperations.count(ownText, "px");
        int val;
        if (c > 5)
            val = -30;
        else
...
//                print("REMOVE:", child);
    ...
    private boolean isAdImage(String imageUrl) {
        return StringOperations.count(imageUrl, "ad") >= 2;
    }
...
//            title = title.substring(0, index + 1);
        ...
        return StringOperations.innerTrim(res.toString());
    }
}
